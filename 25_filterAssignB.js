...
class Employee {
  ...
  let salarySum = 0;
  for (const employee of wiproEmployee) {
    salarySum += employee.emp_salary    
  }
  console.log(`Average salary from wipro : ${salarySum/wiproEmployee.length}`);

console.log(`3) Find the average salary of employee from companies Wipro or Ingy`);

const wiproOrInfySalary = arrayEmployees.filter((employee)=>{
  return employee.emp_company === 'Wipro' || employee.emp_company ==='Infy'
})
let averageSalary = 0;
for (const employee of wiproOrInfySalary) {
  averageSalary += employee.emp_salary
}
 console.log(`Average salary from Wipro or Infy is : ${averageSalary/wiproOrInfySalary.length}`);